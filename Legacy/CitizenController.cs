...
public class CitizenController : MonoBehaviour
{
   ...
   // private GameObject CitizenList;
    ...
    IEnumerator Debuginfo()
    {
        for (; ; )
        {
            //Debug.Log("Park INfo:" + GlobalVars.g_Buildings[12].building);
            //Debug.Log("Park: closed:" + GlobalVars.g_Buildings[12].Closed + " ,Limit:" + GlobalVars.g_Buildings[12].CitizenLimit + " ,inside:" + GlobalVars.g_Buildings[12].NowInsideCount);
    ...
    private void ChooseBuilding()
    {
        ...
        if (BuildingsVisited < maxBuildingVisits)
        {
            ...
            while (((Target == Building) || (GlobalVars.g_Buildings[num].Closed == true)) && loopCount < 30)
            {
                loopCount += 1;
            ...
            else
            {
                ...
                //Debug.Log("id " + num);
                //Debug.Log(GlobalVars.g_Buildings[num].Closed);
                //Debug.Log(GlobalVars.g_Buildings[num].CitizenLimit);
            }
        }
        else { ChooseExit(); }
    ...
    private void OnTriggerEnter2D(Collider2D obj)
    {
        ...
        else if (obj.gameObject == Target.gameObject && entering == true)
        {
            ...
            if (Target.name == "BuildingPark")
            {
                ...
                Debug.Log("CLOSED?" + "inside" + GlobalVars.g_Buildings[buildingNum].NowInsideCount + " , limit:" + GlobalVars.g_Buildings[buildingNum].CitizenLimit);

            }

            //Check building restricitions
            if (GlobalVars.g_Buildings[buildingNum].Closed == true || (GlobalVars.g_Buildings[buildingNum].NowInsideCount >= GlobalVars.g_Buildings[buildingNum].CitizenLimit && GlobalVars.g_Buildings[buildingNum].CitizenLimit != 0) )
            {
                if (Target.name != "BuildingPark")
                {
                    rb.velocity *= 0;
        ...
        if (obj.tag == "OuterLimit")// && exitingcity == true)
        {
            //Debug.Log("Exited City");
            //Destroy(gameObject);
            rb.velocity = new Vector2(0, 0);
            gameObject.GetComponent<SickController>().enabled = false;
            exitingcity = false;
    ...
    private void BackToCity()
    {
        ...
        {
            if (GlobalVars.g_Buildings[i].Closed) { openBuildings--; }
        }
